        ...
        for video, task in result_list:
            ...
            for i, label in enumerate(tmp_label):
                ...
                if '-1' in label.split(',') or '-5' in label.split(',') or not os.path.isfile(tmp_img_path):
                    continue
                self.total_list += [(i, tmp_img_path, self.convert_label(label, task), task)]
        self.img_path_list = [x[1] for x in self.total_list]
        self.idx_list = [x[0] for x in self.total_list]
        self.label_list = [x[2] for x in self.total_list]
        self.task_list = [x[3] for x in self.total_list]
...
if __name__ == '__main__':
    tmp = dataloader(type='valid', batch_size=64)
    tot_time = time.time()
    st_time = time.time()
